...
class ShipToRepo @Inject constructor(@ApplicationContext private val appContext: Context, private val db : FirebaseFirestore){
    ...
    suspend fun createOrder(listOfProducts : List<Product>, listOfCounts : List<Int>) : DefaultStates = withContext(Dispatchers.IO){
        ...
        val totalPrice = if (product.hasOffer) product.offerPrice * listOfCounts[i] else product.price * listOfCounts[i]
        val order = Order(docId,listOfProducts[i].id,totalPrice,product.name,product.merchantId,customerModel.id!!,customerModel.phone!!,customerModel.location!!,customerModel.fullName!!,listOfCounts[i],product.images?.get(0))
        db.collection("customerOrders").document(customerModel.id).collection("Orders").document(docId).set(order).await()
        db.collection("merchantOrders").document(listOfProducts[i].merchantId).collection("Orders").document(docId).set(order).await()
        db.collection("inCart").document(customerModel.id).collection("Products").document(product.id).delete().await()
        db.collection("inCart").document(customerModel.id).collection("Count").document(product.id).delete().await()
       }
